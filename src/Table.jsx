...
export default function Table({ head, body, setUsers }) {
    ...
    return (
                ...
                <div className=" mt-4 w-full border border-zinc-700 rounded">
                    <table className="w-full">
                        ...
                        {filteredData.length === 0 &&
                            <div className="italic text-slate-400 m-5">We couldn't find any results for
                                "{search}"</div>}
                        </tbody>
                    </table>
                </div>
                ...
                <div>
                    <Formik
                        ...
                        onSubmit={(values) => {
                            ...
                            isNameExist.includes(true) || isEmailExist.includes(true) ? alert(`${isNameExist.includes(true) && isEmailExist.includes(true) ? "Name And Email" : isNameExist.includes(true) ? "Name" : "Email"} already exist!`) : users.push({fullName: values.name, email: values.email, age: values.age})
                            ...
                            <Form>
                                <div>
                                    <div className="flex mt-4 gap-x-10">
                                        ...
                                        <button type="submit"
                                                className="bg-orange-400 p-1 rounded w-16 h-9 my-auto">add
                                        </button>
                                    </div>
                                    {errors.name && touched.name ? (
                                        <div
                                            className="text-white bg-red-500 w-full p-3 mt-4 opacity-90 rounded">{errors.name}</div>
                                    ) : null}
                                    {errors.age && touched.age ? (
                                        <div
                                            className="text-white bg-red-500 w-full p-3 mt-4 opacity-90 rounded">{errors.age}</div>
                                    ) : null}
                                    {errors.email && touched.email ? (
                                        <div
                                            className="text-white bg-red-500 w-full p-3 mt-4 opacity-90 rounded">{errors.email}</div>
                                    ) : null}
                                </div>
                            </Form>
                        )}
        ...
        ) : <TableMobile head={head} filteredData={filteredData} search={search} setSearch={setSearch} sorting={sorting}
                         setSorting={setSorting} setUsers={setUsers} isEditModeActive={isEditModeActive} setIsEditModeActive={setIsEditModeActive}/>
    )
}